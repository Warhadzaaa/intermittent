...
candidate1 = Candidate.create(first_name: "Jean-Jacques", last_name: "Albert", sector: "TV production", role:"Production Manager", skills: "Management, organisation", experiences: "Production Manager for La Famille Belier", description: " Production manager on several tv productions in France like La famille Bélier, Les femmes du 6ème étage", address: "Paris", user: user1, avatar: File.open(Rails.root.join('app/assets/images/albert_jean-jacques.png')))


candidate2 = Candidate.create(first_name: "Nadine", last_name: "Chaussonnière", sector: "TV production", role:"Production Manager", skills: " Gestion, Management", experiences: " TV-Show: Bad Banks, Poison sur LM", description:"Looking for my next interesting project", address: "Paris", user: user2, avatar: File.open(Rails.root.join('app/assets/images/nadine.png')))


candidate3 = Candidate.create(first_name: "Christophe", last_name: "Lechat", sector: "TV production", role:"Chief Editor", skills: "Final cut Pro, Adobe premiere, Avid", experiences: "Video journalist / Editor at Mediawan", description:" I am really proud of my reviews. Let's have a look", address: "Paris", user: user3, avatar: File.open(Rails.root.join('app/assets/images/christophe_lechat.png')))


candidate4 = Candidate.create(first_name: "Cassandre", last_name: "Picot", sector: "Movie production", role:"Make-up artist", skills: "TV and cinema make-up ", experiences: "Make", description:" I'm doing the make-up for almost every tv-program on M6", address: "Paris", user: user4, avatar: File.open(Rails.root.join('app/assets/images/cassandre_picot.png')))


candidate5 = Candidate.create(first_name: "Frederic", last_name: "Claude", sector: "TV production", role:"Cameraman", skills: "live recording", experiences: "Iqui cus minient, officid ebiscid quiandemolor", description:"I've spent more than a decade at the AB Group in France", address: "Paris", user: user5, avatar: File.open(Rails.root.join('app/assets/images/fred_claude.png')))


candidate6 = Candidate.create(first_name: "Laura", last_name: "Jaud", sector: "Live performance", role:"Live coordinator", skills: "Management", experiences: "10 years working for Live Nation France", description:" I was in charge of the Lollapolooza Festival in Paris", address: "Paris", user: user6, avatar: File.open(Rails.root.join('app/assets/images/laira_jaud.png')))


candidate7 = Candidate.create(first_name: "Julien", last_name: "Keita", sector: "Cultural center", role:"Campus Manager", skills: "Management, gestion, Excel", experiences: "Campus Manager at le Cube", description:" After managing several centers in Paris, I relocated in Orleans ", address: "Orleans", user: user7, avatar: File.open(Rails.root.join('app/assets/images/julien.png')))


candidate8 = Candidate.create(first_name: "Philippe", last_name: "Diop", sector: "Animation movie", role:"Set designer", skills: "After Effects, Photoshop, Visual effects", experiences: "4 years as set designer at Illumination Mac Guff", description:"I worked on Sing 2", address: "Paris", user: user8, avatar: File.open(Rails.root.join('app/assets/images/philippe_diop.png')))


candidate9 = Candidate.create(first_name: "Myriam", last_name: "Gilles", sector: "Audio-book publishing", role:"Audio production Manager", skills: "Management, Marketing strategy", experiences: "12 years working at Audible studios", description:"I'm used to work on big budget audio production", address: "Paris", user: user9, avatar: File.open(Rails.root.join('app/assets/images/myriam_gilles.png')))

candidate10 = Candidate.create(first_name: "Dominique", last_name: "Besnehard", sector: "TV production", role:"Executive producer", skills: "Casting, Management ", experiences: "Five seasons of 10 pour cent", description:"After a life as a casting director, I've decided to jump behind the scene", address: "Paris", user: user10, avatar: File.open(Rails.root.join('app/assets/images/dominique.png')))
...
company1 = Company.create(name: "UGC CINE CITE", description: "le premier cinéma au monde en termes de fréquentation et la première salle Art & essai de France", siret: "34780600200023", address: "Neuilly-sur-seine", user: user11, avatar: File.open(Rails.root.join('app/assets/images/logo_ugc_2001-2018.svg.png')))
company2 = Company.create(name: "PATHE", description: "Acteur historique du cinéma, Pathé est aujourd’hui premier studio de cinéma français et leader de l’exploitation de salles en Europe continentale", siret: "30758286600041", address: "Neuilly-sur-seine", user: user12, avatar: File.open(Rails.root.join('app/assets/images/pathe_logo.svg.png')))
company3 = Company.create(name: "JEAN-MARC DUMONTET PRODUCTION", description: "Acteur incontournable de la scène humoristique et théâtrale en France, Jean-Marc Dumontet est à la fois producteur d’humoristes et de pièces de théâtre et propriétaire de plusieurs salles de spectacles à Paris.", siret: "38842707200029", address: "Paris", user: user13, avatar: File.open(Rails.root.join('app/assets/images/logo_jmdp.png')))
...
puts "Task and chatroom created"
